RSpec.describe 'network retries' do
  # Port 1 on localhost refuses immediately, so both examples stay offline.
  ...
  it 'retries an unreachable download url with backoff before failing' do
    run = run_mitamae('url_download_retry')
    expect_mitamae_failure(run, %r{gpg download key: url: http://127\.0\.0\.1:1/takumin\.gpg})
    expect(run.log.scan('retrying in').size).to eq(2)
    expect(File.exist?(temporary('unreachable.gpg.asc'))).to be(false)
  end
end
