...
| `homedir`     | String | -- (throwaway)            | GnuPG homedir to keep across runs. Doubles as a key store. |
...
| `:delete` | Removes the file if it exists. No key is read or fetched. |
...
Imports use `--import-options import-minimal` as well. That is a defense,
not redundancy: keys come from arbitrary URLs, and importing a key padded
with tens of thousands of third-party signatures otherwise hangs the run.
...
Network fetches (both `curl` and `--receive-keys`) are retried up to 3 times
...
keep in yours never end up in the file. A `gpg.conf` in it is ignored as
well (`--no-options`): options like `armor` would otherwise decide what gets
written.
...
any `*.conf`, the agent sockets, ...) or anything inside one of its
directories (`public-keys.d`, `private-keys-v1.d`, ...) stops the run; any
other path in there works and is warned about once. Both paths are resolved
before they are compared, so a symlink cannot present one of those as
something else. Keep the keyring outside the homedir and the question does
not arise.
